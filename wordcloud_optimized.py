...
from svgPro.colorStop import stopMax
from Improve.color import color


FILE = os.path.dirname(__file__)
FONT_PATH = os.environ.get('FONT_PATH', os.path.join(FILE, 'DroidSansMono.ttf'))
STOPWORDS = set(stopMax)
...
if __name__ == '__main__':
    ...
    redIM = Image.new(mode='RGB', color=color.getRGB('white'), size=(500, 500))
    pinkIM = Image.new(mode='RGB', color=color.getRGB('black'), size=(100, 100))

    redIM.paste(pinkIM, (100, 100))
    redIM.show()

    res = _get_bolean_mask(mask=redIM)
    print(res)